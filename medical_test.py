...
class Medical_Test:
    ...
    def add_medical_test(self):
        ...
        self.result_and_diagnosis = st.text_area('Result and diagnosis') or 'Test result awaited'
        self.description = st.text_area('Description') or None
    ...
    def update_medical_test(self):
        id = st.text_input('Enter Medical Test ID to update')
        if not id:
            return
    ...
    def delete_medical_test(self):
        id = st.text_input('Enter Medical Test ID to delete')
        if not id:
            return
        if not verify_medical_test_id(id):
            st.error('Invalid Medical Test ID')
    ...
    def medical_tests_by_patient(self):
        patient_id = st.text_input('Enter Patient ID to view all their medical test records')
        if not patient_id:
            return
        if not patient.verify_patient_id(patient_id):
            st.error('Invalid Patient ID')
